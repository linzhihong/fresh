(function($, window) {
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			footer: '<span class="mui-preview-txt"></span>'
		}, options || {});
		this.init();
		this.initEvent();
	};
	...
	proto.init = function() {
		...
		this.footer = this.element.querySelector($.classSelector('.preview-footer')).querySelector($.classSelector('.preview-txt'));
		this.addImages();
	};
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			...
			if(!self.currentGroup[slideNumber].txt){
				self.currentGroup[slideNumber].txt == '';
				self.footer.style.display = 'none';
			}else{
				self.footer.style.display = 'block';
			}
			self.footer && (self.footer.innerText = self.currentGroup[slideNumber].txt);
	...
	proto.addImage = function(img) {
		...
		var src = img.getAttribute('data-preview-src');
		if (img.__mui_img_data && img.__mui_img_data.src === src) { //已缓存且图片未变化
			...
			var imgObj = {
				...
				sLeft: 0,
				sScale: 1,
				txt:img.getAttribute('data-preview-txt'),
				el: img
			};
	...
	proto.refresh = function(index, groupArray) {
		...
		if(!groupArray[index].txt){
			groupArray[index].txt = '';
			this.footer.style.display = 'none';
		}else{
			this.footer.style.display = 'block';
		}
		this.footer && (this.footer.innerText = groupArray[index].txt);
...
//			var wstyle = plus.webview.currentWebview().getStyle();
//			wt = wstyle.top;
//			wb = wstyle.bottom;
//			if(typeof window.previeImageOpen == 'undefined'){
//				window.previeImageOpen = function(){
//					//记录当时打开时页面高度
//					plus.webview.hide(plus.webview.currentWebview(),'none');
//					plus.webview.currentWebview().setStyle({
//						top:'0px',
//						bottom:'0px' 
//					})
//					plus.webview.show(plus.webview.currentWebview(),'none');
//				}
//			}
//			window.previeImageOpen();
//		}
		...
		jQuery('.mui-preview-image.mui-fullscreen').css({
...
//			if(typeof window.previeImageClose == 'undefined'){
//				window.previeImageClose = function(){
//					if(wt!='0px'||wb!='0px'){
//						plus.webview.hide(plus.webview.currentWebview(),'none');
//						plus.webview.currentWebview().setStyle({
//							top:wt,
