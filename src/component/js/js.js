		...
		_this.create = function(){
			...
			if(type=='list'){
				...
				data = $.extend({
					...
					deceleration:window.deceleration//阻尼系数
				},data);
				mui('#'+pid).scroll(data);
...
//	    };
	...
	function reay(){
		...
		$('[component="template"]').each(function(i){
			$(this).Template(i);
		})
		
		$('[component="upload"]').each(function(i){
			$(this).Upload(i);
		})
		
		//所有组件遍历完成
		for(var c = 0; c < System.componentFuns.length ; c++){
			System.componentFuns[c]();
		}
	}
